...
export function setupSocketsServer(server: io.Server) {
  ...
  botService.setCommandResultHandler((result) => {
    ...
    let message = result.data?.toString();
    if (result.error) {
  ...
  server.on("connect", async (socket) => {
    ...
    socket.on("join-chat-room", async (m: any) => {
      if (
        !validateMessage(
          socket,
          Joi.object({
      ...
      const messages = (await chatMessageRepository.getLastChatRoomMessages(
        room
      )).map((x) => ({ username: x.getUsername(), message: x.getMessage(), timestamp: x.getCreatedOn() }));
